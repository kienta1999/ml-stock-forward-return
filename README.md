...
uv run python scripts/backtest.py --leverage 1.35     # lever up (default 1.0 = cash-only); borrow charged at 6.13% APR — see "Vol-target overlay"
...
2021→2026-06 panel (vol-target variant, margin @ 6.13% APR modeled):
...
| `--leverage`     | Gross exposure multiplier (default 1.0 = cash-only). >1.0 borrows on margin (~6.13% APR).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
- **No leverage by default** — exposure caps at 1.0, only scales down. Opt in
  ...
  accrue margin interest daily at `--margin-rate` (default 6.13% APR = IBKR Lite
  USD, flat across tiers) so the reported CAGR nets out the cost of leverage. Leverage lifts
